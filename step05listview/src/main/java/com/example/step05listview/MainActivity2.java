...
public class MainActivity2 extends AppCompatActivity
    ...
    DialogInterface.OnClickListener listener=new DialogInterface.OnClickListener() {
        ...
        public void onClick(DialogInterface dialogInterface, int i) {
            //눌러진 버튼이 Negative 버튼인지 Positive 버튼인지 구별할 숫자값이 매개변수 i 에 전달된다.
            if(i == DialogInterface.BUTTON_POSITIVE){//네 버튼
                //필드에 저장된 값을 활용해서 데이터를 삭제
                names.remove(selectedIndex);
                adapter.notifyDataSetChanged();
            }else if( i == DialogInterface.BUTTON_NEGATIVE){ // 아니요 버튼튼
    ...
    public boolean onItemLongClick(AdapterView<?> adapterView, View view, int i, long l) {
        ...
        String name=names.get(i);
        //위에 필드에 값을 넣어줄때 사용한 익명 클래스에서 참조 가능 하도록 필드에 담아 둔다.
        selectedIndex=i;
